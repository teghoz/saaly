...
COPY ["./Saaly/Saaly.API.csproj", "Saaly.API/"]

RUN dotnet restore "Saaly.API/Saaly.API.csproj"
COPY ./ ./
WORKDIR "/src/Saaly.API"
RUN dotnet build "Saaly.API.csproj" -c Release -o /app/build

FROM build AS publish
RUN dotnet publish "Saaly.API.csproj" -c Release -o /app/publish

FROM base AS final
WORKDIR /app
COPY --from=publish /app/publish .
ENTRYPOINT ["dotnet", "Saaly.API.dll"]