...
COPY ["./Saaly/Saaly.User.csproj", "Saaly.User/"]

RUN dotnet restore "Saaly.User/Saaly.User.csproj"
COPY ./ ./
WORKDIR "/src/Saaly.User"
RUN dotnet build "Saaly.User.csproj" -c Release -o /app/build

FROM build AS publish
RUN dotnet publish "Saaly.User.csproj" -c Release -o /app/publish

FROM base AS final
WORKDIR /app
COPY --from=publish /app/publish .
ENTRYPOINT ["dotnet", "Saaly.User.dll"]