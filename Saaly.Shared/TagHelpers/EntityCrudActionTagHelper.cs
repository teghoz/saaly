...
{
    public class EntityCrudActionTagHelper : TagHelper
    {
        private readonly IUrlHelper _urlHelper;

        public EntityCrudActionTagHelper(IUrlHelper urlHelper)
        {
            _urlHelper = urlHelper;
        }

        /// <summary>
        /// EntityGuid is required to build the route for a specific entity
        /// </summary>
        ...
        public override void Process(TagHelperContext context, TagHelperOutput output)
        {
            ...
            if (ShowEdit)
            {
                if (ShowEditText)
                {
                    builder.Append($@"<a href='{_urlHelper.Page(EditUrl, new { Guid, EntityGuid })}' class='btn btn-sm btn-alt-secondary'>{IconGenerator("edit")} {EditText}</a>");
                }
                else
                {
                    builder.Append($@"<a href='{_urlHelper.Page(EditUrl, new { Guid, EntityGuid })}' class='btn btn-sm btn-alt-secondary'>{IconGenerator("edit")}</a>");
                }
            }
            ...
            if (ShowDetails)
            {
                if (ShowDetailText)
                {
                    builder.Append($@" <a href='{_urlHelper.Page(DetailsUrl, new { Guid, EntityGuid })}' class='btn btn-sm btn-alt-secondary'>{IconGenerator("detail")} {DetailsText}</a>");
                }
                else
                {
                    builder.Append($@" <a href='{_urlHelper.Page(DetailsUrl, new { Guid, EntityGuid })}' class='btn btn-sm btn-alt-secondary'>{IconGenerator("detail")}</a>");
                }
            }
            ...
            if (ShowDelete)
            {
                if (ShowDeleteText)
                {
                    builder.Append($@" <a href='{_urlHelper.Page(DeleteUrl, new { Guid, EntityGuid })}' class='btn btn-sm btn-alt-secondary'>{IconGenerator("delete")} {DeleteText}</a>");
                }
                else
                {
                    builder.Append($@" <a href='{_urlHelper.Page(DeleteUrl, new { Guid, EntityGuid })}' class='btn btn-sm btn-alt-secondary'>{IconGenerator("delete")}</a>");
                }
            }
