...
{
    public class AuditEntityUserTrigger : IAfterSaveTrigger<EntityUser>
    {
        readonly SaalyContext _context;
        public AuditEntityUserTrigger(SaalyContext context)
        {
            _context = context;
        }

        public async Task AfterSave(ITriggerContext<EntityUser> context, CancellationToken cancellationToken)
        {
            var audit = new AuditEntityUser
            {
                IsActive = true,
            ...
            await _context.AuditEntityUsers.AddAsync(audit);
            await _context.SaveChangesAsync();
        }

        public void AfterSave(ITriggerContext<EntityUser> context)
        {
